...
Run = {
   "number" : 5, #the number used to identify which run this is. Changing to new number will create a new directory for all the output in "./results"
...
VideoProcessing = {
   ...
   "generate video?": False, #generate a video for every video that's processed. Set to false for faster run time
...
SignalComparison = {
   "use processed?" : False, #set to false if you want to run the test cases on unproessed data 
   ...
   "run test sets?": True, 
   "test sets" : { #each dict is considered a test set
      "angle_test_cases" : #similarity test cases, ensuring consistency across angles
      { #each of these is considered a testcase
