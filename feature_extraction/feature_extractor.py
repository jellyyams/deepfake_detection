...
class FeatureExtractor(object):
    ...
    anchor_landmark : int
        ...
        indices depends on the landmark extraction model used
    ...
    frame_num : int
        The number of the frame currently being processed by the FeatureExtractor
    ...
    face_detector : face detection object
    ...
    def __init__(self, input_video,  output_directory, anchor_landmark, target_landmarks, dist_display_win_size, bbox_norm, draw_all_landmarks, draw_landmark_nums, draw_anchor_target_connector, three_d_dist, landmark_model, initial_detect, initial_bbox_padding, display_dim):
        """
        ...
        The rest of the inputs initialize the attributes above of the same name (as described above)
        """
        ...
        if self.input_capture is not None:    
            ...
            if self.landmark_model == 'mp':
                self.output_vid_name = 'mp_' + input_vid_name
            else:
                self.output_vid_name  = 'fan_' + input_vid_name
            
            if self.initial_detect:
                self.output_vid_name = 'initdet_' + self.output_vid_name
        else:
            raise ValueError("Invalid input video")

  
        print('---- Setting up output video ----')
        self.out_vid = cv2.VideoWriter(f'{output_directory}/{self.output_vid_name}.mp4', cv2.VideoWriter_fourcc(*'MP4V'), 15, (self.display_dim*3, self.display_dim))
    ...
    def track_landmarks(self, landmark_coords=None):
        raise NotImplementedError("Method track_landmarks() must be implemented in child class.")
    
    def get_output_vid_name(self):
        return self.output_vid_name
            ...
            if ret:
                ...
                if frame is None: 
                    ...
                    if len(landmark_list) == 0:
                        ...
                        #should i be passing aligned_landmark_list_3d or aligned_landmark_list_2d to update landmark_tracker is the question...
    ...
    def resize_img(self, img):
        """
        Resize image to a square with dimensions self.display_dim X self.display_dim
        https://stackoverflow.com/questions/57233910/resizing-and-padding-image-with-specific-height-and-width-in-python-opencv-gives
        """
    ...
    def annotate_frame(self, frame, landmark_list,  initial_face_bbox=None):
        """
        annotate input video frame with bounding box (if initial face detection was performed) and original (unaligned) landmarks
        """
    ...
    def annotate_blank(self, aligned_landmark_list):
        """
        annotate blank canvas with aligned landmarks
        """
